...
require("lazy").setup({
  ...
  -- Add indent-blankline plugin for indent lines
  {
  ...
  -- Replace vim-autoformat with conform.nvim for better silent formatting
  {
    ...
    config = function()
      require("conform").setup({
        ...
        format_on_save = {
          timeout_ms = 500,
          lsp_fallback = true,
          quiet = true,  -- No prompts or messages
        },
      })
    end,
  },
...
-- === Additional background color override ===
...
-- === ToggleTerm and compile/run C++ (with auto-format and save) ===
...
function _G.ToggleCompileRun()
  ...
  if ft == "cpp" then
    -- Auto-save and format before compiling
    vim.cmd("silent! w")  -- Save file silently
    
    -- Wait a moment for formatting to complete, then compile
